...
def _create_mock_exam_table(df, table_type, student_id):
    ...
    ] + [html.Th(jp) for jp in col_headers_jp] + [html.Th("操作", style={'width': '100px'})]
    ...
    for _, row in df_display.iterrows():
        cells = [
            ...
            html.Td(row['exam_date'].strftime('%Y-%m-%d') if pd.notna(row['exam_date']) else '-'),
        ]
        # 点数列を追加 (NaNやNoneは'-'にする)
        for col in score_cols:
            score = row[col]
            # pandas 2.x 以降では pd.isna() が推奨される
            display_score = '-' if pd.isna(score) else int(score) # 整数で表示
            cells.append(html.Td(display_score))

        # 操作ボタンを追加
        cells.append(html.Td([
            dbc.Button("編集", id={'type': 'edit-mock-exam-btn', 'index': row['id']}, size="sm", className="me-1"),
            # 削除ボタンはモーダル内に移動したのでここでは不要かも？ → やっぱりテーブルにもあった方が便利
...
def register_past_exam_callbacks(app):
    ...
    def display_delete_past_exam_confirmation(delete_clicks, result_id):
         # delete_clicks はリストなので any() でチェック
        if not any(nc > 0 for nc in delete_clicks if nc is not None) or result_id is None:
            raise PreventUpdate
        return True
    ...
    def display_delete_past_exam_confirm_from_table(delete_clicks):
        ctx = callback_context
        if not ctx.triggered or not any(nc > 0 for nc in delete_clicks if nc is not None):
            raise PreventUpdate
    ...
    # (display_delete_acceptance_confirmation - 変更なし)
    @app.callback(
